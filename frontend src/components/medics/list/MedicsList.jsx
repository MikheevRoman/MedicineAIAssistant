...
import cl from "./MedicsList.module.css";
...
const MedicsList = observer(({ medics, isInstitution = false }) => {
    ...
    const handleCheckboxChange = () => {
        console.log("Нажал на чек бокс")
        isInstitution ? checkboxStore.inistitution = medics.id : checkboxStore.doctor = medics.id;
        if (isInstitution) {
            checkboxStore.totalSelectedTime = medics.duration;
            checkboxStore.totalSelectedPrice = medics.price;

            checkboxStore.doctorName = medics.name;
            checkboxStore.doctorCategory = medics.category;
            console.log("Нажал на чек бокс для доктора")
        } else {
            checkboxStore.institutionName = medics.name;
            checkboxStore.institutionAddress = medics.address;
            console.log("Нажал на чек бокс для медорганизации")
        }
    ...
    const handleInfoClick = () => {
        if (!isInstitution) {
            navigate(`/infoInstitution/${medics.id}`);
            tg.MainButton.hide();
        } else {
            navigate(`/info/${medics.id}`);
            tg.MainButton.hide();
        }
    }
                ...
                <div className={cl.elem}>
                    <div className={cl.containerForCheckbox}>
                        <input
                            ...
                            id={medics.id}
                            className={cl.chbx}
                            checked={isInstitution ? medics.id === checkboxStore.inistitution : medics.id === checkboxStore.doctor}
                    ...
                    <div className={cl.textbtndiv}>
                        <div className={cl.text}>
                            ...
                            <HeadlineBody className={cl.hdlnbd}>{medics.name}</HeadlineBody>
                            {isInstitution ? <Subheadline1 className={cl.sbhdln}>{medics.price + " ₽ / " + formatMinutesToTimeString(medics.duration)}</Subheadline1>
                                : <Subheadline1 className={cl.sbhdln}>{medics.address}</Subheadline1>
                            }
                        </div>
